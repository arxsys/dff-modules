...
int pff::export_note(libpff_item_t* note, int note_index, Node* parent, bool clone)
{
  ...
  result = libpff_message_get_utf8_subject_size(note, &subject_string_size, &(this->pff_error));
  if (result != 0 && result != -1 && subject_string_size > 0)
  {
    char*	subject = (char*)malloc(sizeof(char *) * subject_string_size);
    libpff_message_get_utf8_subject(note, (uint8_t*)subject, subject_string_size, &(this->pff_error));
...
int pff::export_task(libpff_item_t* task, int task_index, Node* parent, bool clone)
{
  ...
  result = libpff_message_get_utf8_subject_size(task, &subject_string_size, &(this->pff_error));
  if (result != 0 && result != -1 && subject_string_size > 0)
  {
    char*	subject = (char*)malloc(sizeof(char *) * subject_string_size);
    libpff_message_get_utf8_subject(task, (uint8_t*)subject, subject_string_size, &(this->pff_error));
...
int pff::export_contact(libpff_item_t* contact, int contact_index, Node* parent, bool clone)
{
  ...
  result = libpff_message_get_utf8_subject_size(contact, &subject_string_size, &(this->pff_error));
  if (result != 0 && result != -1 && subject_string_size > 0)
  {
    char*	subject = (char*)malloc(sizeof(char *) * subject_string_size);
    libpff_message_get_utf8_subject(contact, (uint8_t*)subject, subject_string_size, &(this->pff_error));
...
int pff::export_appointment(libpff_item_t* appointment, int appointment_index, Node* parent, bool clone)
{
  ...
  result = libpff_message_get_utf8_subject_size(appointment, &subject_string_size, &(this->pff_error));
  if (result != 0 && result != -1 && subject_string_size > 0)
  {
    char*	subject = (char*)malloc(sizeof(char *) * subject_string_size);
    libpff_message_get_utf8_subject(appointment, (uint8_t*)subject, subject_string_size, &(this->pff_error));
...
int pff::export_email(libpff_item_t* email, int email_index, Node *parent, bool clone)
{
  ...
  result = libpff_message_get_utf8_subject_size(email, &subject_string_size, &(this->pff_error));
  if (result != 0 && result != -1 && subject_string_size > 0)
  {
    char*	subject = (char*)malloc(sizeof(char *) * subject_string_size);
    libpff_message_get_utf8_subject(email, (uint8_t*)subject, subject_string_size, &(this->pff_error));
    messageName << std::string(subject);
    free(subject);
  }    
  else
	...
	*attached_item = NULL;
	if (libpff_attachment_get_item(attachment, attached_item, &(this->pff_error)) == 1)
	{
	  ...
	  PffNodeFolder* folder = new PffNodeFolder(attachmentName.str(), parent, this);		
          ...
          if (libpff_item_get_type(*attached_item, &item_type, &(this->pff_error)) == 1)
            if (item_type != LIBPFF_ITEM_TYPE_APPOINTMENT)
	      libpff_item_free(attached_item, &(this->pff_error)); //didn't free because can't clone appointment
	}
	else
	{
