...
pff::~pff()
{
  ...
  if (libpff_file_close(this->pff_file, &pff_error) != 1)
    check_error(pff_error);
  if (libpff_file_free(&(this->pff_file),  &pff_error) != 1)
    check_error(pff_error);
}
...
void    pff::create_recovered(void)
{
  ...
  if (libpff_file_recover_items(this->pff_file, 0, &pff_error) != 1)
  {
    check_error(pff_error) 
    return ;
  }
  if (libpff_file_get_number_of_recovered_items(this->pff_file, &number_of_recovered_items, &pff_error) != 1)
  {
    check_error(pff_error)
    return ;
  }
  if (number_of_recovered_items > 0)
  {
     ...
     {
       if (libpff_file_get_recovered_item(this->pff_file, recovered_item_iterator, &pff_recovered_item, &pff_error) == 1)
       {
...
void	pff::create_orphan()
{
  ...
  if (libpff_file_get_number_of_orphan_items(this->pff_file, &(number_of_orphan_items), &pff_error) != 1)
  {
    check_error(pff_error)
    return ;
  }
  if (number_of_orphan_items > 0)
  {
     ...
     {
        if (libpff_file_get_orphan_item(this->pff_file, orphan_item_iterator, &pff_orphan_item, &(pff_error)) == 1)
        {
...
{
   PffNodeUnallocatedBlocks*  unallocatedPage = new PffNodeUnallocatedBlocks(std::string("unallocated page blocks"), NULL, this, this->parent, LIBPFF_UNALLOCATED_BLOCK_TYPE_PAGE, &(this->pff_file));
   this->registerTree(this->parent, unallocatedPage);

   PffNodeUnallocatedBlocks*  unallocatedData = new PffNodeUnallocatedBlocks(std::string("unallocated data blocks"), NULL, this, this->parent, LIBPFF_UNALLOCATED_BLOCK_TYPE_DATA, &(this->pff_file));
...
void pff::create_item()
{
   ...
   if (libpff_file_get_root_folder(this->pff_file, &pff_root_item, &(pff_error)) != 1)
   {   
      check_error(pff_error)
...
void pff::initialize(Node* parent)
{
  ...
  this->pff_file = NULL;
  pff_error = NULL;
  if (libpff_file_initialize(&(this->pff_file), &(pff_error)) != 1)
  {
    check_error(pff_error)
  ...
  if (libpff_file_open_file_io_handle(this->pff_file, handle, LIBPFF_OPEN_READ, &(pff_error)) != 1)
  {
    check_error(pff_error)
